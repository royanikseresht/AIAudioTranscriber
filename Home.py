...
def main():
    ...
    st.set_page_config(
        ...
        menu_items={
            'Get Help': 'https://github.com/royanikseresht',
            'Report a bug': "https://github.com/royanikseresht",
...
def grab_youtube_video(url: str, input_dir: pathlib.Path):
    ...
    try:
        ...
        ydl_opts = {
            ...
            'outtmpl': str(input_dir / 'audio.%(ext)s'),
        ...
        print(e)
        st.error("😿 Failed to fetch audio from YouTube. Please check the URL or try another video.")
    except Exception as e:
        print(e)
        st.error("😿 An unexpected error occurred while fetching audio from YouTube.")
...
def grab_online_video(url:str,INPUT_DIR:pathlib.Path):
    ...
    try:
        ...
        with open(st.session_state["file_path"], "wb") as f:
            f.write(r.content)
        print("Succesfully fetched the remote audio 💪🏻")
    except:
        st.error("Failed to fetch the audio file 😿")
